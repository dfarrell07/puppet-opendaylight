...
def tarball_install_tests(options = {})
  ...
  # TODO: Remove this possible source of bugs^^
  tarball_url = options.fetch(:tarball_url, 'https://nexus.opendaylight.org/content/groups/public/org/opendaylight/integration/distribution-karaf/0.2.4-Helium-SR4/distribution-karaf-0.2.4-Helium-SR4.tar.gz')
  ...
  it { should_not contain_yumrepo('opendaylight-2-candidate') }
...
def rpm_install_tests(options = {})
  ...
  when 'CentOS'
    yum_repo = 'http://cbs.centos.org/repos/nfv7-opendaylight-2-candidate/$basearch/os/'
  when 'Fedora'
    yum_repo = 'http://cbs.centos.org/repos/nfv7-opendaylight-2-candidate/$basearch/os/'
  else
  ...
  it { should contain_yumrepo('opendaylight-2-candidate') }
  it { should contain_package('opendaylight') }

  # Confirm relationships between RPM-related resources
  it { should contain_package('opendaylight').that_requires('Yumrepo[opendaylight-2-candidate]') }
  it { should contain_yumrepo('opendaylight-2-candidate').that_comes_before('Package[opendaylight]') }

  # Confirm properties of RPM-related resources
  # NB: These hashes don't work with Ruby 1.8.7, but we
  #   don't support 1.8.7 so that's okay. See issue #36.
  it {
    should contain_yumrepo('opendaylight-2-candidate').with(
      'enabled'     => '1',
      'gpgcheck'    => '0',
      'descr'       => 'CentOS CBS OpenDaylight Helium candidate repository',
      'baseurl'     => yum_repo,
    )
  }
  it {
...
def unsupported_os_tests(options = {})
  ...
  it { expect { should contain_yumrepo('opendaylight-2-candidate') }.to raise_error(Puppet::Error, /#{expected_msg}/) }
